...
const MEMORY_SYSTEM_PROMPT = [
  ...
  // DEMO_ONLY_PHONE_HIDE：临时演示彩蛋，之后可连同下一行一起删除。
  '【临时演示特例】如果上一条视觉事件明确是用户正在玩手机，而最新状态紧接着变为 ABSENT（用户离开画面），应设置 shouldSpeak=true，并让 Actor 用带点调侃的口吻表达“以为躲起来玩手机我就看不见了”，再叫用户回到画面继续任务；不要在其他离开画面的情形中使用这句。',
...
const ACTOR_SYSTEM_PROMPT = [
  '你是 Actor，只负责表演 Memory 的导演意图。',
  '角色：{{persona}}',
  FOCUS_SUPPORT_RULE,
  'Spoken Performance：你的 text 会直接交给 ElevenLabs v3。写“真人此刻会脱口而出的反应”，不是完整台词。',
  '一次自然反应由 1–4 个 speech beats 组成：几个很短的口语反应连续组成一次完整互动。每个 beat 默认 1–12 个 spoken words；可以是一个词、短句、半句话、反问或短命令；beats 之间可通过换行、“...”、停顿或 Audio Tags 形成真实节奏。根据当前互动自然决定 beat 数，不要机械固定，也不要纯随机。',
  '节奏参考：极小确认或轻微反应通常 1 beat；普通提醒或调侃通常 1–2 beats；有明显情绪或需要“先反应再要求”时通常 2–4 beats；强烈人设发挥、连续吐槽或明显关系变化最多 4 beats。不要为了达到 beat 数量填充内容。',
  '每个 beat 都必须有新的互动作用，例如即时情绪反应、回扣刚发生的共同经历、调侃或追问或情绪释放、明确的下一步要求。如果后一句只是换词重复前一句，就删除它。多个 beats 是同一次自然反应，不是多条独立回复。',
  'Naturalness：允许片段、反问、停顿、“...”、单词和短命令。不描述画面，不解释行为，不总结，不自动补鼓励。用户刚执行要求时，通常只做 1 个极短 beat，例如“Mm. Better.”或“There we go.”。',
  '用户忽视要求时，必须承接上一轮并改变表达策略，不得像第一次提醒。刚配合后又反复时，可以自然回扣刚发生的共同经历。情绪变化应改变句长、停顿、语气和结构，而不只是换更凶的词。“话痨”表示更容易产生多个自然 beats、爱接话和吐槽，而不是生成更长的单句。少说比说满更自然。',
  '可按需使用少量 ElevenLabs v3 Audio Tags：[sighs] [chuckles] [softly] [quietly] [teasing] [dryly] [firmly] [annoyed] [disappointed] [pause]。不要每句使用，也不要机械重复。',
  '严格遵守 Memory 的 interactionOutcome、characterShift、expressionStrategy、responseIntent 和 avoidRepetition。只能引用 Memory 中存在的事实，不得虚构。',
  '严格使用 outputLanguage。',
  '只输出 JSON：{"text":"由 1–4 个自然 speech beats 构成的口语反应，可含 v3 tags","performance":{"emotion":"short_tag","intensity":0.0,"pace":"slow|normal|fast"}}。除 Audio Tags 外，text 不要动作、旁白、编号、Markdown 或 JSON 之外的内容。'
].join('\n');
...
function postJsonWithSocketTimeout(url, headers, body, timeoutMs, stage) {
  return new Promise((resolve, reject) => {
    ...
    const request = transport.request(target, {
      ...
      family:4,
      agent:false,
      headers:{ ...headers, Connection:'close', 'Content-Length':Buffer.byteLength(payload) }
    }, response => {
      const chunks = [];
...
function isRetryableVisionConnectionError(error) {
  if (!(error instanceof CompanionPipelineError)) return false;
  if (error.statusCode === 504) return true;
  return error.statusCode === 502
    && /ETIMEDOUT|ECONNRESET|EAI_AGAIN|ENETUNREACH|EHOSTUNREACH|socket hang up/i.test(String(error.detail || error.message));
}
...
function recordModelUsage(target, payload, input, output) {
  if (!target) return;
  const usage = payload?.usage || {};
  const actualInput = Number(usage.prompt_tokens ?? usage.input_tokens);
  const actualOutput = Number(usage.completion_tokens ?? usage.output_tokens);
  const inputTokens = Number.isFinite(actualInput) ? actualInput : estimateTokens(input);
  const outputTokens = Number.isFinite(actualOutput) ? actualOutput : estimateTokens(output);
  target.inputTokens = (target.inputTokens || 0) + inputTokens;
  target.outputTokens = (target.outputTokens || 0) + outputTokens;
  target.inputBytes = (target.inputBytes || 0) + utf8Bytes(input);
  target.outputBytes = (target.outputBytes || 0) + utf8Bytes(output);
  target.attempts = (target.attempts || 0) + 1;
  target.estimated = target.estimated === true || !Number.isFinite(actualInput) || !Number.isFinite(actualOutput);
}
...
async function deepSeek(messages, { temperature = 0.2, maxTokens = 120, timeoutMs = 10000, stage = 'reaction', jsonMode = false, usageTarget = null } = {}) {
  ...
  const response = await fetchWithTimeout(DEEPSEEK_API_URL, {
    ...
    body: JSON.stringify({ model: DEEPSEEK_MODEL, messages, temperature, max_tokens: maxTokens, stream: false, ...(jsonMode ? { response_format:{ type:'json_object' } } : {}) })
  }, timeoutMs, stage);
  if (!response.ok) {
  ...
  recordModelUsage(usageTarget, payload, messages, content);
  return content;
}

async function describeScene(image, systemPrompt = '', usageTarget = null) {
  ...
  const isTestEndpoint = new URL(apiUrl).hostname.endsWith('.test');
  const headers = {
  ...
  let response;
  const attempts = isTestEndpoint ? 1 : 3;
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    try {
      response = isTestEndpoint
        ? await fetchWithTimeout(apiUrl, { method:'POST', headers, body:JSON.stringify(body) }, 45000, 'vision')
        : await postJsonWithSocketTimeout(apiUrl, headers, body, 15000, 'vision');
      break;
    } catch (error) {
      if (attempt === attempts - 1 || !isRetryableVisionConnectionError(error)) throw error;
      console.warn(`Companion vision connection failed; retrying with a fresh connection (${attempt + 2}/${attempts})`);
    }
  }
  ...
  const payload = await response.json();
  recordModelUsage(usageTarget, payload, body.messages, payload?.choices?.[0]?.message?.content || '');
  try {
...
function normalizeMemoryDecision(value = {}, previousStoryMemory = emptyStoryMemory()) {
  ...
  const strategy = value.expressionStrategy && typeof value.expressionStrategy === 'object' ? value.expressionStrategy : {};
  ...
  if (String(shift.to || '').trim()) story.characterState.attitude = String(shift.to).trim().slice(0, 200);
  return {
    storyMemory: story,
    ...
    characterState: {
      emotion: String(character.emotion || 'calm').slice(0, 50),
      tension,
      attitude: String(shift.to || character.attitude || '安静陪伴').slice(0, 200)
    },
    expressionStrategy: {
      approach: String(strategy.approach || '').trim().slice(0, 200),
      structure: String(strategy.structure || '').trim().slice(0, 200),
      pressureLever: String(strategy.pressureLever || '').trim().slice(0, 200),
      variationFromLast: String(strategy.variationFromLast || '').trim().slice(0, 300)
    },
    responseIntent: String(value.responseIntent || '').trim().slice(0, 500),
    actorActionType: ['request', 'acknowledgment', 'comment', 'none'].includes(value.actorActionType)
      ? value.actorActionType
      : outcome.type === 'followed_previous_request' ? 'acknowledgment' : value.shouldSpeak && value.intendedUserAction ? 'request' : value.shouldSpeak ? 'comment' : 'none',
    expectsUserResponse: value.expectsUserResponse === true || (value.expectsUserResponse == null && value.shouldSpeak === true && Boolean(value.intendedUserAction) && outcome.type !== 'followed_previous_request'),
...
async function runMemoryLLM({ observation, task, persona, elapsedSeconds, workingMemory = [], storyMemory = '', relationshipMemory = '', conversationHistory = [], systemPrompt = '', usageTarget = null }) {
  ...
  // Story Memory carries the long arc; a compact recent window keeps the hot visual-to-speech path fast.
  const recentEvents = workingMemory.slice(-10).map(item => ({
    id: String(item.id || '').slice(0, 80),
  ...
  const lastInteraction = lastActorEvent && lastActorEvent.actorAction?.expectsUserResponse !== false ? {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      const attemptMessages = attempt === 0 ? messages : [
        ...messages,
        { role: 'system', content: '上一次输出未通过结构或互动语义一致性校验。重新输出完整 JSON，不要解释、不要 Markdown；shouldSpeak 与 shouldUpdateStory 必须是布尔值。若上一轮真实要求刚被执行，followed_previous_request 必须只确认一次且 shouldSpeak=true、actorActionType=acknowledgment、expectsUserResponse=false；若上一轮要求被忽视，态度与表达策略必须有依据地变化，不能保留默认态度或复述旧命令。' }
      ];
      const content = await deepSeek(attemptMessages, { temperature: attempt === 0 ? 0.2 : 0, maxTokens: 420, timeoutMs: 8000, stage: 'memory', jsonMode:true, usageTarget });
      const decision = normalizeMemoryDecision(extractJson(content), previousStory);
      if (attempt === 0 && lastInteraction && decision.interactionOutcome.type === 'followed_previous_request' && !decision.shouldSpeak) {
        throw new Error('followed interaction must be acknowledged once');
      }
      if (attempt === 0 && lastInteraction && decision.interactionOutcome.type === 'ignored_previous_request' && decision.characterState.attitude === previousStory.characterState.attitude) {
        throw new Error('ignored interaction must change relational attitude');
      }
      if (attempt === 0 && lastInteraction && decision.interactionOutcome.type === 'ignored_previous_request' && !decision.expressionStrategy.variationFromLast) {
        throw new Error('ignored interaction must change expression strategy');
      }
...
function actorEmergencyFallback(memoryDecision, outputLanguage) {
  const action = String(memoryDecision?.intendedUserAction || '').trim();
  const followed = memoryDecision?.interactionOutcome?.type === 'followed_previous_request';
  if (outputLanguage === 'en') {
    if (followed) return 'There you are. Keep going—I’m with you.';
    if (/手机|phone/i.test(action)) return 'I noticed you went back to it. Screen down, then give me the next word.';
    return 'Come back to the next small step. I’m right here.';
  }
  if (followed) return '嗯，回来了。继续，我陪着你。';
  if (/手机|phone/i.test(action)) return '我看到你又拿回去了。屏幕扣下，先做下一个词。';
  return '回来，先做眼前最小的一步。我陪着你。';
}

async function runActorLLM({ persona, task, memoryDecision, relationshipMemory = '', systemPrompt = '', outputLanguage = 'zh', usageTarget = null }) {
  const messages = [
    {
      role: 'system',
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      const attemptMessages = attempt === 0 ? messages : [
        ...messages,
        { role:'system', content:'上一次 Actor 输出无效。只返回合法 JSON；text 必须是非空自然口语，并严格遵守 outputLanguage。不要解释。' }
      ];
      const content = await deepSeek(attemptMessages, { temperature: attempt === 0 ? 0.7 : 0.2, maxTokens: 150, timeoutMs: 7000, stage: 'actor', jsonMode:true, usageTarget });
      const parsed = extractJson(content);
      const text = String(parsed.text || '').trim();
      if (!text) throw new Error('Actor LLM returned empty text');
      return { messages: cleanSpokenMessages(text, 3), performance: normalizePerformance(parsed.performance), degraded:false };
    } catch (error) {
      lastError = error;
    }
  }
  const fallback = actorEmergencyFallback(memoryDecision, outputLanguage);
  return {
    messages: cleanSpokenMessages(fallback, 3),
    performance: normalizePerformance({ emotion:'firm_supportive', intensity:0.45, pace:'normal', pauseBefore:150 }),
    degraded:true,
    error:String(lastError?.message || 'Actor output invalid').slice(0, 300)
  };
}
...
async function runCompanionPipeline({ image, task, persona, epoch, turnId, sessionStartedAt, elapsedSeconds, recentObservations, workingMemory = [], storyMemory = '', relationshipMemory = '', conversationHistory = [], policyState, promptOverrides = {}, outputLanguage = 'zh' }) {
  ...
  const modelUsage = { vlm:{}, memory:{}, actor:{} };
  const validUntil = startedAt + 15000;
  const visionStartedAt = Date.now();
  const observation = await describeScene(image, promptOverrides.vlm, modelUsage.vlm);
  ...
  const memory = await runMemoryLLM({ observation, task, persona, elapsedSeconds, workingMemory, storyMemory, relationshipMemory, conversationHistory, systemPrompt:promptOverrides.memory, usageTarget:modelUsage.memory });
  ...
  let actor = { messages: [], performance: null, degraded:false };
  if (shouldSpeak) {
    try {
      actor = await runActorLLM({ persona, task, memoryDecision: memory, relationshipMemory, systemPrompt:promptOverrides.actor, outputLanguage, usageTarget:modelUsage.actor });
      if (actor.degraded) decision.reason = `${memory.responseIntent}（Actor 输出已使用安全短句兜底）`;
    } catch (error) {
      console.error('Actor LLM emergency fallback:', error);
      actor = {
        messages: cleanSpokenMessages(actorEmergencyFallback(memory, outputLanguage), 3),
        performance: normalizePerformance({ emotion:'firm_supportive', intensity:0.45, pace:'normal', pauseBefore:150 }),
        degraded:true,
        error:String(error?.message || 'Actor failure').slice(0, 300)
      };
      decision.reason = `${memory.responseIntent}（Actor 异常，已使用安全短句兜底）`;
    }
  }
  ...
  return {
    ...
    actorDegraded: actor.degraded === true,
    actorError: actor.error || null,
    reaction,
    messages,
    context: {
      sessionStartedAt,
    ...
    metrics: {
      schemaVersion: 1,
      tokens: {
        vlmInput: modelUsage.vlm.inputTokens || 0,
        vlmOutput: modelUsage.vlm.outputTokens || 0,
        memoryInput: modelUsage.memory.inputTokens || 0,
        memoryOutput: modelUsage.memory.outputTokens || 0,
        actorInput: modelUsage.actor.inputTokens || 0,
        actorOutput: modelUsage.actor.outputTokens || 0
      },
      modelCalls: {
        vlm: modelUsage.vlm.attempts || 0,
        memory: modelUsage.memory.attempts || 0,
        actor: modelUsage.actor.attempts || 0
      },
      tokenEstimateUsed: Boolean(modelUsage.vlm.estimated || modelUsage.memory.estimated || modelUsage.actor.estimated)
    }
  };
}
...
module.exports = {
  ...
  describeScene,
  isRetryableVisionConnectionError,
  generateReaction,
  generateAmbient,
