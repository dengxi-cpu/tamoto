...
async function generateAmbient({ type, task, persona, elapsedSeconds, activity, scene, state }) {
  ...
  return cleanSpokenMessages(await deepSeek([
    { role: 'system', content: `你是${persona || '安静陪伴用户的角色'}。需要称呼时只能叫“大小姐”，不要使用姓名，也不要每句都称呼。只输出${sentenceCount}句连续的自然口语短句，每句不超过15个汉字；像真人随口说话，意思自然推进，不要重复；不要引号、动作、旁白、编号或Markdown。` },
