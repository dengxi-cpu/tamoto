...
async function describeScene(image, systemPrompt = '', usageTarget = null) {
  ...
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    try {
      ...
      if (attempt === attempts - 1 || !(error instanceof CompanionPipelineError) || error.statusCode !== 504) throw error;
      console.warn(`Companion vision timeout; retrying with a fresh connection (${attempt + 2}/${attempts})`);
    }
  }
