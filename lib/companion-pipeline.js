...
function cleanSpokenMessages(text, sentenceCount) {
  ...
  }).filter(Boolean).map(value => value.slice(0, 80));
}
...
async function classifyScene(scene, uncertain) {
  ...
  const content = await deepSeek([
    { role: 'system', content: '根据客观画面描述判断用户状态。只输出 JSON：{"state":"STUDYING|PHONE|RESTING|ABSENT|OTHER"}。PHONE 仅限明确在看或操作手机；看电脑不算。不要解释。' },
    { role: 'user', content: scene }
  ], { temperature: 0, maxTokens: 30 });
  try {
    const state = String(extractJson(content).state || '').toUpperCase();
    return ['STUDYING', 'PHONE', 'RESTING', 'ABSENT', 'OTHER'].includes(state) ? state : 'OTHER';
  } catch (_) {
    return 'OTHER';
  }
}
...
function evaluateSpeechPolicy({ state, policyState = {}, nowMs = Date.now() }) {
  ...
  if (state === 'STUDYING') {
    next.focusStreakStartedAt = previousState === 'STUDYING' && previous.focusStreakStartedAt
      ? Number(previous.focusStreakStartedAt) : nowMs;
  } else if (previousState === 'STUDYING' && previous.focusStreakStartedAt) {
  ...
  if (state === 'STUDYING') reason = '用户正在认真学习，尽量不打扰';
  if (state === 'PHONE') {
    const phoneSeconds = Math.max(0, Math.floor((nowMs - next.phoneStartedAt) / 1000));
    const thresholdSeconds = next.earnedBreakUntil && nowMs < next.earnedBreakUntil ? 180 : 60;
    const globalCooldown = next.lastAnySpokenAt && nowMs - next.lastAnySpokenAt < 3 * 60 * 1000;
    const repeatCooldown = next.phoneEventSpokenAt && nowMs - next.phoneEventSpokenAt < 10 * 60 * 1000;
    if (phoneSeconds < thresholdSeconds) reason = `连续玩手机${phoneSeconds}秒，未达到${thresholdSeconds}秒阈值`;
    else if (globalCooldown) reason = '距离上次发言不足3分钟';
    else if (repeatCooldown) reason = '同一次玩手机事件仍在10分钟冷却期';
    else {
      shouldSpeak = true;
...
async function generateReaction({ scene, task, persona, sessionStartedAt, elapsedSeconds, recentObservations = [] }) {
  const sentenceCount = randomSpeechCount();

  const systemPrompt = [
    '你是正在通过视频陪伴用户专注的角色。',
    `角色人设：${persona || '毒舌但关心用户的陪伴者'}`,
    '根据摄像头刚刚看到的客观画面，自然地对用户说一句话。',
    '你必须结合本轮专注已经持续的时间和最近观察理解上下文：长时间认真后短暂休息应体谅或轻松提醒；刚开始不久就分心可以更直接。不要把单帧画面孤立判断。',
    `要求：只输出${sentenceCount}句连续的口语短句，每句6到22个汉字；像真人自然接着说，不要重复意思；不要输出引号、动作、旁白、标签、编号或分析；只能评论画面描述明确提供的事实，不得把不确定对象具体化。`
  ].join('\n');
...
async function generateAmbient({ type, task, persona, elapsedSeconds, activity, scene, state }) {
  ...
  return cleanSpokenMessages(await deepSeek([
    { role: 'system', content: `你是${persona || '安静陪伴用户的角色'}。只输出${sentenceCount}句连续的自然口语短句，每句6到20个汉字，意思要自然推进，不要重复；不要引号、动作、旁白、编号或Markdown。` },
  ...
  const content = await deepSeek([
    {
      ...
      content: [
        ...
        `用户刚刚主动对你说话，请直接自然回应。只输出${sentenceCount}句适合语音播放的口语短句，每句6到25个汉字；句子之间自然承接，不要重复；不要引号、动作、旁白、编号、列表或Markdown；不要凭空声称看到了画面没有提供的事实。`
      ].join('\n')
    },
    {
      role: 'user',
