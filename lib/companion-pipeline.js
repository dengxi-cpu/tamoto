...
async function deepSeek(messages, { temperature = 0.2, maxTokens = 120, timeoutMs = 10000, stage = 'reaction', jsonMode = false } = {}) {
...
async function runMemoryLLM({ observation, task, persona, elapsedSeconds, workingMemory = [], storyMemory = '', relationshipMemory = '', conversationHistory = [], systemPrompt = '' }) {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      ...
      const content = await deepSeek(attemptMessages, { temperature: attempt === 0 ? 0.2 : 0, maxTokens: 420, timeoutMs: 8000, stage: 'memory', jsonMode:true });
...
async function runActorLLM({ persona, task, memoryDecision, relationshipMemory = '', systemPrompt = '', outputLanguage = 'zh' }) {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      ...
      const content = await deepSeek(attemptMessages, { temperature: attempt === 0 ? 0.7 : 0.2, maxTokens: 150, timeoutMs: 7000, stage: 'actor', jsonMode:true });
