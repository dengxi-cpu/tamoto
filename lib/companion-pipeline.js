...
const MEMORY_SYSTEM_PROMPT = [
  ...
  '如果用户无视上一句提醒并继续相同行为，可以承接上下文再次开口，但必须换一个具体角度。只能引用记忆里真实存在的任务、进度和用户承诺，禁止编造。',
...
const ACTOR_SYSTEM_PROMPT = [
  ...
  '当导演要求制止明显分心时，第一句话要短、直接、可执行，例如“手机放下”。可以逐渐施压，但不能侮辱、羞辱或贬低用户。只能引用 Memory LLM 中真实存在的任务、进度和承诺，禁止虚构“你每次都这样”或不存在的数字。',
...
async function describeScene(image, systemPrompt = '') {
  ...
  const visionRequest = new URL(apiUrl).hostname.endsWith('.test')
    ? (url, headers, body) => fetchWithTimeout(url, { method:'POST', headers, body:JSON.stringify(body) }, 35000, 'vision')
