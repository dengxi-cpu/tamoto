    ...
    body: JSON.stringify({
      ...
      messages: [{
        ...
        content: [
          {
            type: 'text',
            text: '你是视频陪伴系统的眼睛。只记录画面中能直接看见的事实，不推测心理、动机、偷懒或逃避。只输出 JSON：{"observation":"一到两句客观描述","visibleFacts":{"phone":false,"gaze":"unknown","hands":"unknown","position":"unknown"},"state":"STUDYING|READING|WRITING|COMPUTER_WORK|PHONE|RESTING|ABSENT|DRINKING|STRETCHING|OTHER|UNKNOWN","confidence":0.0}。state 只用于兼容监控；核心是 observation 和 visibleFacts。只有明确看见手机本体且用户正在看或操作它时 phone 才为 true、state 才为 PHONE。仅仅低头、手在画外或看不清手部不能判 PHONE。'
          },
...
async function runMemoryLLM({ observation, task, persona, elapsedSeconds, workingMemory = [], storyMemory = '', relationshipMemory = '', conversationHistory = [] }) {
  ...
  const messages = [
    {
      role: 'system',
      content: [
        '你是视频陪伴系统的 Memory LLM，也是角色的记忆管理器和导演。你不直接对用户说台词。',
        '结合最近事件、对话、会话故事和长期关系，理解共同经历并决定此刻是否值得开口。不要使用 warning_stage 或机械提醒次数。',
        ...
        '只输出 JSON：{"shouldUpdateStory":false,"evidenceEventIds":["支持本次判断的事件id"],"storyMemory":{"version":1,"sessionGoal":"任务","summary":"会话摘要","importantMoments":[{"id":"事件id","summary":"关键时刻","elapsedSeconds":0}],"openLoops":[{"id":"loop-id","content":"未完成互动","status":"open|resolved","evidenceEventIds":["事件id"]}],"characterState":{"emotion":"英文短标签","tension":"low|medium|high","attitude":"关系态度"},"lastInteraction":{"type":"vision|user_speech|ai_speech|session","summary":"最近互动","elapsedSeconds":0}},"currentSituation":"刚刚发生了什么","behaviorChange":"与之前相比的变化","characterState":{"emotion":"英文短标签","tension":"low|medium|high","attitude":"角色此刻的关系状态"},"responseIntent":"导演意图或沉默原因","shouldSpeak":false,"silentReaction":"silent|listening|watching"}。',
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      ...
      return normalizeMemoryDecision(extractJson(content), previousStory);
    } catch (error) {
      if (attempt === 1) {
...
async function runActorLLM({ persona, task, memoryDecision, relationshipMemory = '' }) {
  const content = await deepSeek([
    {
      role: 'system',
      content: [
        '你是 Actor LLM。Memory LLM 已完成事实分析、关系理解和开口判断；你不要重新判断，只演角色。',
        `角色人设：${persona || '安静陪伴用户的学习搭子'}`,
        FOCUS_SUPPORT_RULE,
...
async function runCompanionPipeline({ image, task, persona, epoch, turnId, sessionStartedAt, elapsedSeconds, recentObservations, workingMemory = [], storyMemory = '', relationshipMemory = '', conversationHistory = [], policyState }) {
  ...
  if (shouldSpeak) {
    try {
      actor = await runActorLLM({ persona, task, memoryDecision: memory, relationshipMemory });
    } catch (error) {
