...
const VISION_SYSTEM_PROMPT = '你是视频陪伴系统的眼睛。只记录画面中能直接看见的事实，不推测心理、动机、偷懒或逃避。只输出 JSON：{"observation":"一到两句客观描述","visibleFacts":{"phone":false,"gaze":"unknown","hands":"unknown","position":"unknown"},"state":"STUDYING|READING|WRITING|COMPUTER_WORK|PHONE|RESTING|ABSENT|DRINKING|STRETCHING|OTHER|UNKNOWN","confidence":0.0}。state 只用于兼容监控；核心是 observation 和 visibleFacts。只有明确看见手机本体且用户正在看或操作它时 phone 才为 true、state 才为 PHONE。仅仅低头、手在画外或看不清手部不能判 PHONE。';
...
async function runActorLLM({ persona, task, memoryDecision, relationshipMemory = '', systemPrompt = '' }) {
  const content = await deepSeek([
    {
      role: 'system',
...
async function runCompanionPipeline({ image, task, persona, epoch, turnId, sessionStartedAt, elapsedSeconds, recentObservations, workingMemory = [], storyMemory = '', relationshipMemory = '', conversationHistory = [], policyState, promptOverrides = {} }) {
  ...
  if (shouldSpeak) {
    try {
      actor = await runActorLLM({ persona, task, memoryDecision: memory, relationshipMemory, systemPrompt:promptOverrides.actor });
    } catch (error) {
