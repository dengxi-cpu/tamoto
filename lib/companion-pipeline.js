...
async function generateReaction({ scene, task, persona, sessionStartedAt, elapsedSeconds, recentObservations = [], triggerReason = '' }) {
  ...
  const systemPrompt = [
    ...
    `要求：只输出${sentenceCount}句连续的口语短句，每句尽量控制在2～18个汉字；必须把句意说完整，绝不能为了满足字数截断半句话，必要时可以略超18字；短一点、自然一点，像真人微信语音；不要重复意思；不要输出引号、动作、旁白、标签、编号或分析；只能评论画面描述明确提供的事实，不得把不确定对象具体化。`
  ].join('\n');
...
async function generateAmbient({ type, task, persona, elapsedSeconds, activity, scene, state }) {
  ...
  return cleanSpokenMessages(await deepSeek([
    { role: 'system', content: `你是${persona || '安静陪伴用户的角色'}。需要称呼时遵循人设中指定的称呼，不要每句都称呼。${FOCUS_SUPPORT_RULE}只输出${sentenceCount}句连续的自然口语短句，每句尽量控制在2～18个汉字；必须把句意说完整，绝不能为了满足字数截断半句话，必要时可以略超18字；像真人随口说话，意思自然推进，不要重复；不要引号、动作、旁白、编号或Markdown。` },
  ...
  const content = await deepSeek([
    {
      ...
      content: [
        ...
        FOCUS_SUPPORT_RULE,
        `用户刚刚主动对你说话，请直接自然回应。需要称呼时遵循人设中指定的称呼，不要每句都称呼。只输出${sentenceCount}句适合语音播放的口语短句，每句尽量控制在2～18个汉字；必须把句意说完整，绝不能为了满足字数截断半句话，必要时可以略超18字；短一点、自然一点，像真人微信语音；句子之间自然承接，不要重复；不要引号、动作、旁白、编号、列表或Markdown；不要凭空声称看到了画面没有提供的事实。`
      ].join('\n')
    },
    {
      role: 'user',
  ...
  const content = await deepSeek([
    {
      ...
      content: [
        ...
        '尽量控制在2～18个汉字，必须把句意说完整，绝不能为了满足字数截断半句话，必要时可以略超18字；不要动作、旁白、引号、编号或Markdown；不要重复询问任务；表达“知道了，我会陪你开始”的感觉，但不要机械复述固定模板。'
      ].join('\n')
    },
