...
async function describeScene(image) {
    ...
    body: JSON.stringify({
      ...
      messages: [{
        ...
        content: [
          {
            type: 'text',
            text: '客观判断画面中用户此刻正在做什么。重点观察姿势、视线和手部动作，不推测心理与动机。只输出 JSON：{"scene":"一到两句客观描述","state":"STUDYING|READING|WRITING|COMPUTER_WORK|PHONE|RESTING|ABSENT|DRINKING|STRETCHING|OTHER|UNKNOWN","confidence":0.0}。PHONE 仅限明确看到手机；ABSENT 仅限画面确实无人；看不清时返回 UNKNOWN 并降低 confidence。'
          },
  ...
  try {
    ...
    const state = String(parsed.state || 'UNKNOWN').toUpperCase();
