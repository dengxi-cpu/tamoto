...
function pickInstantReaction(event, turnId = 0) {
  const lines = INSTANT_REACTIONS[event] || [];
  return lines.length ? [lines[Math.abs(Number(turnId) || 0) % lines.length]] : [];
}

function getTaskReminder(task) {
  const value = String(task || '').trim();
  if (!value) return '你刚刚说今天有任务要完成。';
  return `你刚刚说今天要${value.replace(/[。！？!?]+$/, '')}。`;
}

function getTaskProgressQuestion(task) {
  const value = String(task || '');
  if (/背.*单词|单词.*背/.test(value)) return '现在背了几个？';
  return '现在完成了几个？';
}

function pickPhoneReaction(event, task, phoneReminderCount = 0) {
  if (event === 'PHONE_ENTER') return ['手机放下。'];
  if (event !== 'PHONE_REPEAT') return [];
  if (Number(phoneReminderCount) <= 2) {
    return ['别装。', getTaskReminder(task), getTaskProgressQuestion(task)];
  }
  return ['手机扣下。', '现在。'];
}
...
function stabilizeState(rawState, confidence, policyState = {}) {
  const previousConfirmed = VALID_STATES.has(policyState.confirmedState)
    ? policyState.confirmedState : 'UNKNOWN';
  const normalized = VALID_STATES.has(rawState) ? rawState : 'UNKNOWN';
  const reliable = normalized !== 'UNKNOWN' && Number(confidence) >= 0.55;
  if (!reliable) {
    // UNKNOWN used to preserve PHONE forever. Two consecutive ambiguous frames
    // now release a stale phone state, while the first ambiguous frame merely
    // suppresses speech until the camera has enough evidence again.
    if (previousConfirmed === 'PHONE') {
      const candidateCount = policyState.candidateState === 'UNKNOWN'
        ? (Number(policyState.candidateCount) || 0) + 1 : 1;
      if (candidateCount >= 2) {
        return { state: 'UNKNOWN', candidateState: null, candidateCount: 0, changed: true };
      }
      return { state: previousConfirmed, candidateState: 'UNKNOWN', candidateCount, changed: false };
    }
    return { state: previousConfirmed, candidateState: null, candidateCount: 0, changed: false };
  }
  if (normalized === previousConfirmed) {
    return { state: previousConfirmed, candidateState: null, candidateCount: 0, changed: false };
  }
  const candidateCount = policyState.candidateState === normalized
    ? (Number(policyState.candidateCount) || 0) + 1 : 1;
  const requiredCount = normalized === 'PHONE' && Number(confidence) >= 0.8 ? 1 : 2;
  if (candidateCount < requiredCount) {
    return { state: previousConfirmed, candidateState: normalized, candidateCount, changed: false };
  }
  return { state: normalized, candidateState: null, candidateCount: 0, changed: true };
}
...
function evaluateSpeechPolicy({ state, policyState = {}, nowMs = Date.now() }) {
  ...
  const next = {
    confirmedState: previous.confirmedState || 'UNKNOWN',
    candidateState: previous.candidateState || null,
    ...
    phoneReminderCount: Number(previous.phoneReminderCount) || 0,
  ...
  if (state === 'PHONE') {
    ...
    const awaitingUserResponse = !firstPhoneReminder
      && next.phoneReminderCount >= 2
      && nowMs - Number(next.phoneEventSpokenAt) < 12 * 1000;
    ...
    if (awaitingUserResponse) reason = '已连续提醒两次，等待用户回应或执行';
    else if (phoneSeconds < thresholdSeconds) reason = `连续玩手机${phoneSeconds}秒，未达到${thresholdSeconds}秒阈值`;
    else {
      shouldSpeak = true;
      event = firstPhoneReminder ? 'PHONE_ENTER' : 'PHONE_REPEAT';
  ...
  return { shouldSpeak, reason, state, event, speechMode: event ? 'instant' : 'generated', policyState: next };
}
...
async function describeScene(image) {
    ...
    body: JSON.stringify({
      ...
      messages: [{
        ...
        content: [
          {
            type: 'text',
            text: '客观判断画面中用户此刻正在做什么。重点观察姿势、视线和手部动作，不推测心理与动机。只输出 JSON：{"scene":"一到两句客观描述","state":"STUDYING|READING|WRITING|COMPUTER_WORK|PHONE|RESTING|ABSENT|DRINKING|STRETCHING|OTHER|UNKNOWN","confidence":0.0,"phoneVisible":false}。只有画面中明确看见手机本体，并且用户正在看或操作它时，phoneVisible 才能为 true 且 state 才能为 PHONE。仅仅低头、手在画外、看不清手部、此前见过手机，都不能判 PHONE。ABSENT 仅限画面确实无人；无法判断具体活动时返回 UNKNOWN 并降低 confidence。'
          },
  ...
  try {
    ...
    const phoneVisible = parsed.phoneVisible === true;
...
async function generateAmbient({ type, task, persona, elapsedSeconds, activity, scene, state }) {
  ...
  const instruction = type === 'opening'
    ...
    : type === 'opening_event'
      ...
      : type === 'pause'
        ? '用户刚刚暂停了专注。用一句自然的话接住这个状态，不责备，也不要长篇大论。'
        : type === 'resume'
          ? '用户刚刚继续专注。用一句简短的话陪用户重新进入状态。'
          : type === 'completion'
            ? '用户刚刚结束了这次专注。给一句有具体感、不夸张的收尾回应。'
            : type === 'praise'
...
async function generateDialogue({ text, task, persona, elapsedSeconds, scene, history = [] }) {
  if (/(?:就|只)?玩(?:一会儿|一下|五分钟|5分钟)/.test(String(text || ''))) {
    return ['不行。', '你每次说五分钟，最后都是半小时。', '手机扣下。', '现在。'];
  }
  const sentenceCount = randomSpeechCount();
...
async function runCompanionPipeline({ image, task, persona, epoch, turnId, sessionStartedAt, elapsedSeconds, recentObservations, policyState }) {
  ...
  const stable = stabilizeState(observation.state, observation.confidence, policyState);
  const decision = evaluateSpeechPolicy({
    state: stable.state,
    policyState: {
      ...policyState,
      confirmedState: stable.state,
      candidateState: stable.candidateState,
      candidateCount: stable.candidateCount
    }
  });
  decision.rawState = observation.state;
  decision.confidence = observation.confidence;
  decision.stateChanged = stable.changed;
  decision.phoneVisible = observation.phoneVisible;
  if (decision.state === 'PHONE' && observation.state !== 'PHONE') {
    decision.shouldSpeak = false;
    decision.event = null;
    decision.speechMode = 'silent';
    decision.reason = '当前帧没有明确看到手机，暂停提醒并等待状态确认';
  }
  const reactionStartedAt = Date.now();
  const phoneMessages = decision.shouldSpeak
    ? pickPhoneReaction(decision.event, task, decision.policyState?.phoneReminderCount)
    : [];
  const instantMessages = phoneMessages.length
    ? phoneMessages
    : decision.shouldSpeak ? pickInstantReaction(decision.event, turnId) : [];
  decision.speechMode = instantMessages.length ? 'instant' : decision.shouldSpeak ? 'generated' : 'silent';
  const messages = instantMessages.length ? instantMessages : decision.shouldSpeak ? await generateReaction({
