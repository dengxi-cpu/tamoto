...
function cleanSpokenMessages(text, sentenceCount) {
  ...
  return parts.slice(0, sentenceCount).map(part => {
    ...
    return `${Array.from(body).slice(0, 15).join('')}${punctuation}`;
  }).filter(Boolean);
}
...
async function generateReaction({ scene, task, persona, sessionStartedAt, elapsedSeconds, recentObservations = [] }) {
  ...
  const systemPrompt = [
    ...
    '根据摄像头刚刚看到的客观画面，自然地对用户说话。需要称呼时只能叫“大小姐”，不要使用姓名，也不要每句都称呼。',
    FOCUS_SUPPORT_RULE,
...
async function generateAmbient({ type, task, persona, elapsedSeconds, activity, scene, state }) {
  ...
  return cleanSpokenMessages(await deepSeek([
    { role: 'system', content: `你是${persona || '安静陪伴用户的角色'}。需要称呼时只能叫“大小姐”，不要使用姓名，也不要每句都称呼。${FOCUS_SUPPORT_RULE}只输出${sentenceCount}句连续的自然口语短句，每句不超过15个汉字；像真人随口说话，意思自然推进，不要重复；不要引号、动作、旁白、编号或Markdown。` },
  ...
  const content = await deepSeek([
    {
      ...
      content: [
        ...
        FOCUS_SUPPORT_RULE,
        `用户刚刚主动对你说话，请直接自然回应。需要称呼时只能叫“大小姐”，不要使用姓名，也不要每句都称呼。只输出${sentenceCount}句适合语音播放的口语短句，每句不超过15个汉字；短一点、自然一点，像真人微信语音；句子之间自然承接，不要重复；不要引号、动作、旁白、编号、列表或Markdown；不要凭空声称看到了画面没有提供的事实。`
      ].join('\n')
    },
    {
      role: 'user',
